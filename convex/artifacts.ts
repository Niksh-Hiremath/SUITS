  ...
  handler: async (ctx, args) => {
    ...
    for (let index = 0; index < args.jurorParts.length; index += 1) {
      ...
      await ctx.db.insert("juryVotes", {
        ...
        evidenceIds: args.contradictionFound ? ["E-003"] : [],
    ...
    await ctx.db.insert("debriefs", {
      ...
      contradictions: [{
        description: "Whether Northstar was logged at Gate B before the lighting interruption.",
        status: args.contradictionFound ? "found" : "missed",
        turnCitations: args.contradictionTurnIds,
        evidenceIds: args.contradictionFound ? ["E-003"] : [],
      }],
      evidenceUsed: args.contradictionFound ? [{ evidenceId: "E-003", turnCitations: args.contradictionTurnIds }] : [],
      evidenceMissed: args.contradictionFound ? [] : [{ evidenceId: "E-003", reason: "The Gate B timestamp was not confronted in cross-examination." }],
