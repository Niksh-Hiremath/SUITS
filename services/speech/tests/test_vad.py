...
def test_energy_vad_detects_start_and_end_windows() -> None:
    ...
    first_voice = vad.process(_constant_pcm(1_000), sample_rate_hz=16_000)
    second_voice = vad.process(_constant_pcm(1_000), sample_rate_hz=16_000)
    first_silence = vad.process(_constant_pcm(0), sample_rate_hz=16_000)
    second_silence = vad.process(_constant_pcm(0), sample_rate_hz=16_000)
    third_silence = vad.process(_constant_pcm(0), sample_rate_hz=16_000)
...
def test_energy_vad_flushes_active_speech_and_resets() -> None:
    ...
    observation = vad.process(_constant_pcm(900), sample_rate_hz=16_000)
...
def test_energy_vad_rejects_malformed_pcm(pcm: bytes) -> None:
    ...
    with pytest.raises(ValueError, match="whole signed 16-bit"):
        vad.process(pcm, sample_rate_hz=16_000)


def test_energy_vad_derives_duration_from_pcm_samples() -> None:
    vad = EnergyVad(threshold_rms=500, min_speech_ms=40, end_silence_ms=600)

    one_voice_sample = vad.process(_constant_pcm(1_000, sample_count=1), sample_rate_hz=16_000)
    assert one_voice_sample.speech_started is False
    assert vad.active is False
