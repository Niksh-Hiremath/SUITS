...
- [x] OpenNext and Wrangler configuration with a production-like local `workerd` preview;
...
- 2026-07-20 06:24-08:14 IST - Milestone 8 privacy-safe Court Records projection checkpoint
  - Changed: added strict server-internal readers for generated jury/debrief artifacts and privacy-safe audio lifecycle audits, then added an owner-bound pure Court Records projector. The projector independently replays the complete canonical event stream, binds the exact CaseGraph content/source hashes, reconstructs every role-specific KnowledgeView used by accepted model calls, requires a bijection between AI events and accepted traces, validates final Terra coaching against its original request, and emits only allowlisted payload-free event, transcript, procedure, lifecycle, model, debrief, audio, and integrity DTOs.
  - Privacy boundary: raw event payloads, private opposing strategy, raw jury reasoning, artifact JSON, prompt hashes, KnowledgeView hashes, hidden identifiers outside explicitly labelled debrief strata, raw audio, transcript text in audio rows, timing marks, and provider errors are rejected or discarded before projection. Citation resources use `(kind, identifier)` identity so legitimate cross-namespace ID collisions remain distinct; owner-visible lifecycles are reconstructed from the player-counsel view plus committed public use, preventing caller-supplied scope laundering.
  - Integrity fixes from review: AI provenance now requires `source === "ai"` exactly when model metadata exists; generic trace inputs, accepted event coverage, graph bindings, objection-question citations, judge provenance stripping, interruption/resume audio chains, and debrief transcript-event bindings all mirror canonical writers exactly. The final debrief binding regression uses chronological turn order `turn:z` then `turn:a` and proves the projector reconstructs the writer's lexical binding order.
  - Verified: the final focused projector/speech slice passed five files and 122 tests; the projector file passed 23 adversarial tests; strict root TypeScript, zero-warning scoped ESLint, and `git diff --check` passed. The full root suite initially exposed four stale deterministic debrief fixtures; after separating general active-transcript citations from admitted-proof-only improved-closing citations, the rerun passed 161 files with three intentional live-only skips, 1,448 tests passed, and three skipped. Two independent final audits reported no remaining P0-P3 correctness, privacy, or real-row-compatibility finding.
  - Remaining: the pure projector and strict audit inputs are complete, but browser delivery is not. Next add the owner-bound Convex assembly action and authenticated Next.js BFF, persist prepared browser audio audits through that protected path, then replace the static `/records` notice with list/detail/download views and run the complete Milestone 8 browser gate.
  - Commits: `ee90979`, `1ecd32f`, `7435109`, `e94220f`, `29b39d8`, and `b3771a8`.

- 2026-07-20 08:15-09:38 IST - Milestone 8 owner-bound records and durable audio-audit checkpoint
  - Changed: added owner-bound Convex assembly/list actions that reuse the exact privacy-safe projector, secret-protected service routes, and signed-session Next.js list/read/download BFFs. Browser authority comes only from the signed cookie; strict V3 trial binding, trusted-origin checks, private/no-store headers, allowlisted errors, stable redacted export bytes, and response-to-request binding are enforced at the browser boundary.
  - Audio durability: added a strict metadata-only audio-audit ingest contract and Convex table/mutation. The mutation replays the complete canonical owner-bound trial and applies the exact Court Records audio projector inside the same transaction before insertion, so a schema-valid forged turn, actor, ruling, or interruption cannot poison later records reads. The browser sink is immutable to one validated V3 trial, uses a monotonic epoch clock, serializes each bounded record once, delivers one keepalive request at a time, retries only network/408/425/429/5xx failures with exact bytes, and fails closed on permanent rejection or receipt mismatch.
  - Lifecycle/privacy boundary: raw microphone audio, transcript fragments, timing-mark values, owner IDs, provider errors, and browser-supplied authority never enter the audit request. Presentation unsubscribes before controller teardown, while a separate effect-owned audit listener remains through `controller.close()` so synchronous user and asynchronous playback terminal events are retained. `pagehide`/hidden visibility expedite pending work without closing audio or breaking BFCache; Strict Mode effect instances cannot close or retarget each other's sinks.
  - Verified: the combined controller/audio/page/BFF slice passed six files and 94 tests; both TypeScript projects, scoped zero-warning ESLint, `git diff --check`, and the 20-route production build passed. The full root suite passed 167 files with three intentional live-only skips, 1,480 tests passed, and three skipped. Independent final review repeated five files and 88 tests and reported no blocking privacy, retry, queue, trial-pinning, disconnect, or lifecycle finding.
...
- 2026-07-20 10:21-11:09 IST - Milestone 8 Court Records workspace implementation checkpoint
  - Changed: replaced the static migration notice with an owner-session list/detail workspace driven only through the strict same-origin records client. The async server boundary reduces the untrusted `trial` query to `none`, one exact V3 ID, or content-free `invalid`; the hearing links only a validated current trial. The client cancels obsolete list/detail/download work, refuses stale selection results, downloads the exact validated server JSON string, and revokes temporary object URLs.
  - Review surface: added a fail-closed pure view model that rechecks redundant head/count bindings, validates and iteratively flattens a 20,000-event graph, resolves citations by exact namespace/kind identity, preserves null-versus-zero metrics, and creates fixed bounded pages. The responsive UI mounts one active inspector at a time for overview, transcript, procedure, fact/evidence lifecycles, model calls, audio metadata, citations, Terra coaching, and the flat event ledger. Nested citations/history rows are capped with explicit export disclosure; DTOs now fail closed above 20,000 lifecycle transitions or four model attempts.
  - Semantics: active transcript remains labelled as unstricken history rather than admitted proof; stricken testimony remains audit-visible and jury-excluded; restricted facts remain owner-visible but non-public. Historical strike reasons are never inferred, recovery diagnostics remain redacted, repair/retry is not labelled fallback, unavailable usage/cost is not zero, audio identity binding is not content verification, and raw audio is reported as unretained.
  - Verified: seven focused files passed 69 tests; strict root and Convex TypeScript, zero-warning scoped ESLint, `git diff --check`, the 28-test projector suite, and the 20-route Next.js production build passed. Two independent reviews drove fixes for nested DOM bounds, stale-download side effects, partial-known metric labelling, keyboard focus, mobile header/data overflow, retry paths, pagination/section semantics, and inert citation styling. This is code/build evidence, not yet mounted-browser or full-trial export evidence.
  - Remaining: extend the production-path two-witness Chromium fixture through exact-trial navigation, persisted metadata-only audio, every records panel, byte-stable repeat download, strict v2 parse/deep equality, reload, privacy canaries, and zero page/console errors; then run the complete Milestone 8 gate.

- 2026-07-20 11:10-12:05 IST - Milestone 8 mounted Court Records browser checkpoint
  - Changed: extended the existing production-path two-witness voice fixture through the exact validated hearing link into the owner-bound Records workspace. The fixture waits for three stable projection reads, strictly parses the mounted list/detail responses, visits every bounded inspector, reloads without a hearing command, downloads the exact validated v2 JSON twice, requires byte equality and deep equality with the owner read, and rejects private keys, authored canaries, the exact signed owner-session UUID, raw-audio fields, product alerts, browser errors, and non-cancellation API failures.
  - Gate-driven fixes: the first browser run exposed that one controller generation spans multiple recordings while the audit preparer treated it as one utterance, and that STT can finish without a VAD start. User-audio identity is now `(generation, utteranceId)`, conflicting reuse remains fenced, and one terminal-only record is emitted with `startedAtEpochMs: null` rather than inventing a start. The mounted record consequently contains exactly two completed question audits, one completed closing audit, and nine playback audits. Caller cancellation, service timeout, and transport outage are now separately classified as bounded 499/504/503 outcomes, so a Strict-Effect cleanup is not logged or reported as a Convex outage.
  - Browser proof: the selected flow passed in 2.0 minutes with 12 unique metadata-only audio rows; exact local/transcript/interruption bindings; all nine panels; a flat event page bounded to 50 rows; two byte-identical `court-records-view.v2` downloads; stable projection hash/IDs; no visible text composer; and no product, page, console, or non-cancellation API errors. Valid judge-ruling audio binds to an interruption with no transcript turn, while resumed-witness testimony binds to both the turn and interruption; the test now asserts those distinct canonical semantics.
...
- 2026-07-21 22:11-22:40 IST - Cloudflare Worker and PDF-runtime checkpoint
  - Changed: added the OpenNext Cloudflare adapter, Wrangler Worker configuration, immutable static-asset headers, Cloudflare build/preview/upload/deploy scripts, and the documented Webpack production-build mode. Replaced Node-oriented `pdf-parse` with the serverless `unpdf` PDF.js build while preserving bounded page-by-page extraction, page provenance, cancellation, deadline, character cap, corrupt-input failure, and cleanup behavior.
  - Runtime findings: the first Turbopack OpenNext artifact built but failed under Windows `workerd` with `ChunkLoadError`; the OpenNext-documented Webpack fallback resolved it. The first `pdf-parse` Worker route reached a native `@napi-rs/canvas` path and failed with `DOMMatrix is not defined`; `unpdf` removed that native/runtime dependency. A stale manually started preview later held `.open-next` open; stopping only that exact port-8790 process tree cleared the Windows `EPERM` lock and the clean rebuild passed.
  - Verified: the OpenNext build generated all 20 pages/routes; Wrangler dry-run accepted the Worker at 9,460.06 KiB raw / 1,969.65 KiB gzip; the main local `workerd` preview returned HTTP 200 for `/`, `/cases`, a seeded case detail, `/hearing`, `/records`, and direct-only `/preflight`; and the protected case-session handler loaded and returned its intentional safe 503 when the preview omitted `SUITS_SESSION_SECRET`. The dedicated `npm run cf:test:pdf` harness imported the production adapter into a real local Worker, received zero environment bindings, extracted two exact pages with MIME/label/page provenance, and proved bounded process/port teardown. Focused ingestion/lifecycle tests, TypeScript, and scoped ESLint passed.
  - Boundary: this closes the Worker configuration and text-PDF runtime proof, not the full protected Convex flow. The module-global preflight in-flight promise, peak memory for the current 20 MB multipart boundary, production bindings/secrets, managed speech, and deployed-domain E2E remain open.
  - Commits: `154c283` (`build: add Cloudflare worker target`) is on `origin/main`; the Worker-safe PDF adapter and reproducible runtime harness follow as a separate scoped checkpoint.

- 2026-07-21 22:58-23:02 IST - Worker-safe preflight cache checkpoint
  - Changed: removed the module-global pending preflight promise so one Cloudflare request can never await another request's I/O. The cache now retains only completed schema-bounded readiness JSON; concurrent cache misses own independent probes, and a monotonic plain-data sequence prevents an older/slower result or a post-`clear()` completion from repopulating the cache.
  - Verified: focused cache, server-check, and route suites passed 10 tests; regressions prove concurrent callers receive different promises and each invokes its own probe, the later refresh remains cached when an older one finishes last, and clearing during a pending probe invalidates that completion. Scoped ESLint and root TypeScript passed.
  - Boundary: this closes the known cross-request promise hazard without claiming the remaining protected compile-route memory gate, production bindings, managed speech, or deployment.
  - Commit: the scoped preflight-cache checkpoint follows.
...
- Top-level Court Records pagination alone is not a sufficient browser bound: per-row citations, lifecycle transitions, and model attempts can multiply the mounted DOM. The UI therefore caps nested summaries and points to the exact export, while the strict v2 DTO independently rejects attempt/history arrays beyond their canonical transitive limits.
...
- The former preflight cache shared an in-flight fetch promise at module scope. Cloudflare Workers can reject cross-request I/O reuse, so it now caches only completed, schema-bounded JSON; independent misses never share promises, and a plain numeric sequence prevents stale completions from overwriting the latest cache state.
- `pdf-parse` is not portable to this Worker bundle: its module path loaded native `@napi-rs/canvas` and failed at runtime because `DOMMatrix` was unavailable. Cloudflare's serverless `unpdf` PDF.js build now passes a real two-page local-`workerd` fixture with exact page provenance. The separate 20 MB multipart route's peak memory still must be measured against [Cloudflare's 128 MB Worker memory limit](https://developers.cloudflare.com/workers/platform/limits/) rather than inferred from build success.
- The serverless PDF.js build transfers and detaches its input `Uint8Array`; a focused regression proved that passing the workflow's only packet buffer directly changes its byte length to zero. The adapter must retain a deliberate parser copy because the original bytes are subsequently stored durably. That copy makes a lower PDF-specific byte cap and actual-route memory gate necessary rather than optional.
...
- Treat `source === "ai"` and non-null model metadata as an exact event invariant in Court Records, and key citation resources by both kind and identifier. This prevents provenance relabeling from evading trace coverage while preserving valid fact/evidence namespace collisions.
...
- 2026-07-20 06:24-08:14 IST - Milestone 8 privacy-safe projector verification
  - Strict generated-artifact and audio-audit reader suites passed 18 and 10 tests respectively; the final projector suite passed 23 tests, including canonical replay, owner isolation, namespace collisions, accepted-trace coverage, KnowledgeView reconstruction, final-debrief strata, writer-compatible lexical transcript bindings, and exact interruption audio semantics.
  - `npm exec -- vitest run src/domain/court-records/project.test.ts src/lib/speech/hearing-performance.test.ts src/lib/speech/hearing-policy.test.ts src/lib/speech/hearing-controller.test.ts src/lib/speech/hearing-audio-audit.test.ts --reporter=dot` - exit 0; five files and 122 tests passed.
  - `npm run typecheck -- --pretty false`, scoped Court Records/speech ESLint with `--max-warnings 0`, and `git diff --check` - exit 0. Independent final audits repeated smaller 68-test slices and reported no P0-P3 correctness/privacy finding.
  - First `npm test -- --reporter=dot` run - exit 1; 1,444 tests passed and four deterministic debrief-fixture checks failed because the fixtures still cited stricken turns and reused transcript citations in improved closings after the stricter contract. The failure is retained here rather than misreported as a pass.
  - After correcting only those fixtures, `npm exec -- vitest run convex/hearingRuntime.integration.test.ts src/server/hearing-api/e2e-primary-trial-provider.test.ts --reporter=dot` - exit 0; 42/42 passed. Final `npm test -- --reporter=dot` - exit 0; 161 files passed, three intentional live-only files skipped, 1,448 tests passed, and three skipped.
  - `git push origin main` - exit 0 through `b3771a8`; strict artifact/audio inputs, debrief-fixture compatibility, shared privacy-safe actor aliases, DTO schemas, and the pure projector are on `origin/main`. Owner-bound BFF/UI/browser evidence remains pending and is not claimed by this checkpoint.

- 2026-07-20 08:15-09:38 IST - Milestone 8 owner-bound delivery and audio-audit verification
  - Owner-bound Convex Court Records assembly/list, secret service routes, signed-session list/read/download BFFs, strict audio ingest, idempotent persistence, and adversarial semantic-binding suites passed throughout the atomic commits. `npx convex dev --once` accepted each Convex schema/function change on linked development deployment `cheery-bandicoot-36` without another login prompt.
  - `npm exec -- vitest run src/lib/speech/hearing-audio-audit-sink.test.ts src/lib/speech/hearing-audio-audit.test.ts src/lib/speech/hearing-performance.test.ts src/lib/speech/hearing-controller.test.ts src/app/hearing/page.source.test.ts "src/app/api/hearings/[trialId]/audio-audits/route.test.ts" --reporter=dot` - exit 0; six files and 94 tests passed.
  - `npm run typecheck -- --pretty false` and `npm exec -- tsc -p convex/tsconfig.json --noEmit --pretty false` - exit 0. Scoped ESLint with `--max-warnings 0` and `git diff --check` also exited 0.
  - `npm run build` - exit 0; Next.js 16.2.10 compiled/typechecked and generated all 20 routes, including the private records and audio-audit BFFs.
...
- 2026-07-20 10:21-11:09 IST - Milestone 8 Court Records workspace implementation verification
  - `npm test -- src/components/records/court-records-client.test.ts src/components/records/court-records-pagination.test.ts src/components/records/court-records-view-model.test.ts src/components/records/court-records-workspace.source.test.ts src/app/records/page.test.tsx src/domain/court-records/navigation.test.ts src/app/hearing/page.source.test.ts --reporter=dot` - exit 0; seven files and 69 tests passed. Coverage includes malformed/duplicate query redaction, strict exact-trial URL construction, owner-authority source bans, a 20,000-deep iterative graph, namespace-collision citation resolution, redundant head/count rejection, null-versus-zero metrics, nested/transitive bounds, abort/stale fences, explicit retry paths, one switched panel, keyboard/responsive source contracts, and exact JSON Blob/revocation ordering.
  - `npm test -- src/domain/court-records/project.test.ts --reporter=dot` - exit 0; 28 projector tests passed after the transitive DTO maxima. `npm run typecheck` and `npm exec tsc -- -p convex/tsconfig.json --noEmit --pretty false` - exit 0. Scoped ESLint over the records components/page/domain schema/navigation and hearing-link slice with `--max-warnings 0` - exit 0. Full `npm run lint` exited 0 with only the four existing generated-file unused-disable warnings. `git diff --check` - exit 0.
  - Final combined focused run - exit 0; eight files and 97 tests passed. `npm run build` then exited 0 in 22.5 seconds; Next.js 16.2.10 compiled and typechecked, generated 20/20 pages, and retained `/records` plus its private list/read/download BFF routes as dynamic server routes.
  - Security/correctness review found one P1 and two P2 issues: unbounded nested DOM collections, stale post-navigation file creation, and partial-known cost/latency presented as complete. UX/accessibility review found no P0 and identified focus visibility, narrow-header/long-token overflow, retry, landmark, section-control, empty-copy, and inert-chip issues. The stable snapshot now caps nested summaries, enforces transitive DTO maxima, aborts and commit-synchronously generation-fences downloads before Blob creation, labels known metric coverage, exposes high-contrast focus/retry/landmark/pressed-control semantics, wraps the mobile header, and emergency-wraps record text. Both final targeted re-reviews reported no remaining P0-P2 finding.
  - In-app Chromium rendered the real owner-session error/empty workspace at explicit 390 x 844 and 1280 x 800 viewports. Read-only layout measurements reported document widths 375/1265 within viewport widths 390/1280, the desktop archive rail was exactly 292px, and browser warning/error logs were empty. This does not exercise a selected record or count as the pending full-trial records gate.

- 2026-07-20 11:10-12:05 IST - Milestone 8 mounted Court Records browser verification
  - `npx vitest run src/lib/speech/hearing-audio-audit.test.ts src/lib/speech/hearing-audio-audit-sink.test.ts src/lib/speech/hearing-controller.test.ts src/server/case-api/convex-service.test.ts src/server/court-records/http.test.ts src/app/api/records/route.test.ts convex/hearingAudioAudits.integration.test.ts src/app/api/hearings/[trialId]/audio-audits/route.test.ts` - exit 0; eight files and 109 tests passed. Scoped ESLint, root `npx tsc --noEmit`, and `git diff --check` also exited 0.
  - Earlier gate attempts were intentionally retained as diagnostic evidence: the first mounted run found zero user-speech rows beside nine playback rows; after the lifecycle fix, Convex held 12 rows and the gate exposed an overstrict ruling-turn assertion. Later passes reached all panels/downloads and exposed the Next.js dev route-announcer scope, a drifted schema literal, and an overbroad any-UUID privacy regex. Each assertion was corrected against the strict public contract rather than removed.
  - Read-only linked-deployment inspection of `trial_c077c43f86d2434c8d4192ba9aa8a5de` showed two completed `user_speech/question`, one completed `user_speech/closing`, two local objection playbacks, two interruption-verified judge rulings with null turn IDs, two interruption-and-turn-verified resumed testimony playbacks, and three transcript-turn-verified playbacks. Three repeated projections had the same 12 record IDs and privacy-safe hash.
...
- 2026-07-21 22:11-22:40 IST - OpenNext/Worker/PDF verification
  - `npm run cf:build` - exit 0 after the exact stale manual preview was stopped; OpenNext 1.20.1 built Next.js 16.2.10 through Webpack, typechecked it, generated all 20 pages/routes, and emitted `.open-next/worker.js`. The OpenNext Windows-support warning remains recorded; local runtime proof is therefore required and was executed.
  - `wrangler deploy --dry-run --outdir .wrangler/worker-dry-run` - exit 0; 118 assets read, total Worker upload 9,460.06 KiB raw / 1,969.65 KiB gzip, with the configured self-reference and assets bindings recognized.
  - Main local `workerd` preview - HTTP 200 for `/`, `/cases`, `/cases/redwood-signal-retaliation`, `/hearing`, `/records`, and `/preflight`; the hashed CSS response retained `Cache-Control: public,max-age=31536000,immutable`. `POST /api/cases/session` reached the dynamic handler and returned the intentional `CASE_SESSION_UNAVAILABLE` 503 because that preview deliberately lacked `SUITS_SESSION_SECRET`, rather than crashing during Worker module load.
  - `npm run cf:test:pdf` - exit 0; a dedicated local Worker imported `PDF_EXTRACTION_ADAPTER`, received an explicit empty environment with `bindingNames: []`, extracted `Fictional Harbor filing page one.` and `Page two records the disputed inspection.` as pages 1 and 2 through `unpdf-v1.6.2`, returned `application/pdf`, and released its exact process tree/listening port. A review initially found that Wrangler auto-loaded `.env`/`.env.local`; the final harness supplies an empty env file, disables dotenv/process-env inheritance, and asserts the runtime binding set is empty.
  - `npm exec -- vitest run src/server/case-ingestion/adapters/pdf-lifecycle.test.ts src/server/case-ingestion/adapters/adapters.test.ts src/server/case-ingestion/ingestion.test.ts --reporter=dot` - exit 0; three files and 19 tests passed, including corrupt input, character/page bounds, in-flight cancellation, timeout with stalled cleanup, post-extraction cleanup timeout, and real page provenance. Cleanup is separately capped at two seconds while the primary timeout/cancellation code is preserved. The corrupt fixture emitted PDF.js's expected `Indexing all PDF objects` warning.
  - `npm run typecheck` and scoped ESLint over the adapter and Worker harness - exit 0.
  - `npm test -- --reporter=dot` - exit 0; 179 files passed, three intentional live-only files skipped, 1,581 tests passed, and three skipped. After the Cloudflare build exposed generated `.open-next` vendor files to the root lint traversal, the ESLint global ignore set was extended to cover OpenNext/Wrangler artifacts; `npm run lint` then passed against repository-owned sources.

- 2026-07-21 22:58-23:02 IST - Preflight request-isolation verification
  - `npm exec -- vitest run src/server/preflight/cache.test.ts src/server/preflight/server-checks.test.ts src/app/api/preflight/route.test.ts --reporter=verbose` - exit 0; three files and 10 tests passed. The cache regressions prove no pending promise is shared between callers, only the latest-started successful refresh can update completed cache data, `clear()` fences an older completion, rejection remains uncached, and degraded TTL behavior is preserved.
  - Scoped ESLint over the cache implementation/tests and `npm run typecheck` - exit 0.
  - `npm run cf:build` - exit 0 in 41.6 seconds; OpenNext rebuilt the complete 20-page/route Worker artifact with the request-isolated preflight cache. The documented Windows warning remains, while the prior local-`workerd` runtime evidence and focused request-isolation regressions remain the applicable runtime proof.
