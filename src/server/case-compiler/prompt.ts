...
- Each ownerPath must identify that exact CaseGraph record using numeric array indexes from your returned arrays (for example, caseGraph.parties.0 and caseGraph.witnesses.1.priorStatements.0). Never substitute schema key names such as partyId, witnessId, or priorStatementId into ownerPath. A record-scoped group may cite only provenance owned by that schema record. Prior statements use their own provenance instead of inheriting witness provenance.
