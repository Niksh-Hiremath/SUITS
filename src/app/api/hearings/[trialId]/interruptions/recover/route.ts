...
import { recoverFinalBoundInterruption } from "@/server/hearing-api/final-bound-interruption";
import {
...
export async function POST(
  ...
  if (!isTrustedRequestOrigin(request)) {
    return hearingJsonError(
      403,
      "ORIGIN_REJECTED",
      "Cross-origin hearing recovery is not allowed.",
    );
  }
  ...
  try {
    ...
    console.error("hearing_interruption_recovery_session_failed", {
  ...
  try {
    const resolution = await recoverFinalBoundInterruption({
    ...
    if (
      ...
      error.code === "FINAL_BOUND_INTERRUPTION_INVALID"
    ) {
    ...
    console.error("hearing_interruption_recovery_failed", {
      name: error instanceof Error ? error.name : "UnknownError",
