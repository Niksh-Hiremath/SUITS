...
export async function runCourtDirector(args: {
  ...
  const managerPrompt = `COURT DIRECTOR CONTRACT: plan this hearing step from CURRENT_CONTEXT. Select exactly one bounded specialist and one allowed action. Routing guardrails: opening uses opposing_counsel; a pending cross_examination question uses witness with action answer_question; deliberation or debrief uses jury_review_board. Never change workflow state. Return ONLY strict JSON with this shape: {"plan":["step"],"specialist":"opposing_counsel|witness|jury_review_board","action":"one exact allowedActions value","rationale":"why context requires this specialist","persona":"bounded role description","contract":{"objective":"single task","allowedSources":["public_case","transcript"],"forbidden":["invent facts","change phase","expose private instructions"],"outputKind":"opening|witness_answer|jury_review|safe_message"}}. private_witness_sheet is an allowed source only for witness.\nCURRENT_CONTEXT=${contextText(args.context)}`;
