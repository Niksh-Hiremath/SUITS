import { CaseGraphV1Schema, type CaseGraphV1 } from "./schema";

const SHA256_INITIAL = [
  0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a,
  0x510e527f, 0x9b05688c, 0x1f83d9ab, 0x5be0cd19,
] as const;

const SHA256_ROUND = [
  0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5,
  0x3956c25b, 0x59f111f1, 0x923f82a4, 0xab1c5ed5,
  0xd807aa98, 0x12835b01, 0x243185be, 0x550c7dc3,
  0x72be5d74, 0x80deb1fe, 0x9bdc06a7, 0xc19bf174,
  0xe49b69c1, 0xefbe4786, 0x0fc19dc6, 0x240ca1cc,
  0x2de92c6f, 0x4a7484aa, 0x5cb0a9dc, 0x76f988da,
  0x983e5152, 0xa831c66d, 0xb00327c8, 0xbf597fc7,
  0xc6e00bf3, 0xd5a79147, 0x06ca6351, 0x14292967,
  0x27b70a85, 0x2e1b2138, 0x4d2c6dfc, 0x53380d13,
  0x650a7354, 0x766a0abb, 0x81c2c92e, 0x92722c85,
  0xa2bfe8a1, 0xa81a664b, 0xc24b8b70, 0xc76c51a3,
  0xd192e819, 0xd6990624, 0xf40e3585, 0x106aa070,
  0x19a4c116, 0x1e376c08, 0x2748774c, 0x34b0bcb5,
  0x391c0cb3, 0x4ed8aa4a, 0x5b9cca4f, 0x682e6ff3,
  0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208,
  0x90befffa, 0xa4506ceb, 0xbef9a3f7, 0xc67178f2,
] as const;

function rotateRight(value: number, amount: number): number {
  return (value >>> amount) | (value << (32 - amount));
}

/** Runtime-neutral SHA-256 so domain integrity checks also work in Convex isolates. */
export function sha256Utf8(value: string): string {
  const input = new TextEncoder().encode(value);
  const paddedLength = Math.ceil((input.length + 9) / 64) * 64;
  const padded = new Uint8Array(paddedLength);
  padded.set(input);
  padded[input.length] = 0x80;
  const bitLength = input.length * 8;
  const paddedView = new DataView(padded.buffer);
  paddedView.setUint32(paddedLength - 8, Math.floor(bitLength / 0x1_0000_0000), false);
  paddedView.setUint32(paddedLength - 4, bitLength >>> 0, false);

  const digest: number[] = [...SHA256_INITIAL];
  const words = new Uint32Array(64);
  for (let offset = 0; offset < paddedLength; offset += 64) {
    for (let index = 0; index < 16; index += 1) {
      words[index] = paddedView.getUint32(offset + index * 4, false);
    }
    for (let index = 16; index < 64; index += 1) {
      const previous15 = words[index - 15];
      const previous2 = words[index - 2];
      const small0 = rotateRight(previous15, 7) ^ rotateRight(previous15, 18) ^ (previous15 >>> 3);
      const small1 = rotateRight(previous2, 17) ^ rotateRight(previous2, 19) ^ (previous2 >>> 10);
      words[index] = (words[index - 16] + small0 + words[index - 7] + small1) >>> 0;
    }

    let [a, b, c, d, e, f, g, h] = digest;
    for (let index = 0; index < 64; index += 1) {
      const large1 = rotateRight(e, 6) ^ rotateRight(e, 11) ^ rotateRight(e, 25);
      const choose = (e & f) ^ (~e & g);
      const temporary1 = (h + large1 + choose + SHA256_ROUND[index] + words[index]) >>> 0;
      const large0 = rotateRight(a, 2) ^ rotateRight(a, 13) ^ rotateRight(a, 22);
      const majority = (a & b) ^ (a & c) ^ (b & c);
      const temporary2 = (large0 + majority) >>> 0;
      h = g;
      g = f;
      f = e;
      e = (d + temporary1) >>> 0;
      d = c;
      c = b;
      b = a;
      a = (temporary1 + temporary2) >>> 0;
    }

    digest[0] = (digest[0] + a) >>> 0;
    digest[1] = (digest[1] + b) >>> 0;
    digest[2] = (digest[2] + c) >>> 0;
    digest[3] = (digest[3] + d) >>> 0;
    digest[4] = (digest[4] + e) >>> 0;
    digest[5] = (digest[5] + f) >>> 0;
    digest[6] = (digest[6] + g) >>> 0;
    digest[7] = (digest[7] + h) >>> 0;
  }

  return digest.map((word) => word.toString(16).padStart(8, "0")).join("");
}

export function canonicalizeCaseGraphV1(graphInput: unknown): string {
  return JSON.stringify(CaseGraphV1Schema.parse(graphInput));
}

export function computeCaseGraphContentHash(graphInput: unknown): string {
  return sha256Utf8(canonicalizeCaseGraphV1(graphInput));
}

export function collectCaseGraphProvenanceIds(graphInput: unknown): string[] {
  const graph = CaseGraphV1Schema.parse(graphInput);
  const identifiers = new Set<string>();
  const visit = (value: unknown): void => {
    if (Array.isArray(value)) {
      value.forEach(visit);
      return;
    }
    if (typeof value !== "object" || value === null) return;
    for (const [key, child] of Object.entries(value)) {
      if (key === "provenanceId" && typeof child === "string") {
        identifiers.add(child);
      }
      visit(child);
    }
  };
  visit(graph);
  return [...identifiers].sort((left, right) => left.localeCompare(right));
}

export function assertCaseGraphContentHash(
  graphInput: unknown,
  expectedHash: string,
): CaseGraphV1 {
  const graph = CaseGraphV1Schema.parse(graphInput);
  const actualHash = sha256Utf8(JSON.stringify(graph));
  if (actualHash !== expectedHash) {
    throw new Error(`CASE_GRAPH_CONTENT_HASH_MISMATCH:${expectedHash}:${actualHash}`);
  }
  return graph;
}
